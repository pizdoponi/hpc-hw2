...
run_one() {
    ...
    if [[ "$run_id" -eq 1 ]]; then
        output_path="$outdir/final_states/final_gpu_${size}.pgm"
    fi

    local cmd_output
    if [[ -n "$output_path" ]]; then
        cmd_output=$(./lenia.out gpu "$size" "$steps" "$output_path")
    else
        cmd_output=$(./lenia.out gpu "$size" "$steps")
    fi
    ...
    if [[ -z "$total" ]]; then
        echo "Failed to parse T_TOTAL for gpu size=$size run=$run_id" >&2
        echo "$cmd_output" >&2
        exit 1
    fi

    printf "%s,GPU,%s,%s\n" "$size" "$run_id" "$total" >> "$raw_csv"
    printf "[GPU] size=%s run=%s T_TOTAL=%s\n" "$size" "$run_id" "$total"
}
